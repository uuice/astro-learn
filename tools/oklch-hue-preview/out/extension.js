...
// src/extension.ts
var mapOklchToSrgb = toGamut("rgb", "oklch");
var toRgb = converter_default("rgb");
function extractHueFromDocument(text) {
  const m = text.match(/--hue:\s*([\d.]+)\s*;/);
  if (!m) return null;
  const n = parseFloat(m[1]);
  return Number.isFinite(n) ? n : null;
}
function resolveHueComponent(raw, baseHue) {
  const t = raw.trim();
...
function oklchToVsColor(l, c2, h) {
  const ok = { mode: "oklch", l, c: c2, h };
  const rgb3 = toRgb(mapOklchToSrgb(ok));
  if (!rgb3 || rgb3.mode !== "rgb") return null;
  const { r, g, b } = rgb3;
  const ch = (x) => {
    const v = x > 1 ? x / 255 : x;
    return Math.min(1, Math.max(0, v));
  };
  return new vscode.Color(ch(r), ch(g), ch(b), rgb3.alpha ?? 1);
}
...
function collectOklchCalls(text) {
  const out = [];
  let searchFrom = 0;
  const lower = "oklch(";
  while (searchFrom < text.length) {
    ...
    const hRaw = m[3].trim();
    out.push({ start: idx, end: closeParen + 1, l, c: c2, hRaw });
    searchFrom = closeParen + 1;
  }
  return out;
}
var CSS_LIKE = /* @__PURE__ */ new Set(["css", "scss", "less", "postcss", "tailwindcss"]);
function provideDocumentColors(document) {
